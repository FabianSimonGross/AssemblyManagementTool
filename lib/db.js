...
async function main () {
  ...
  await executeSqlQueryWithoutValues(
    'CREATE TABLE IF NOT EXISTS ' + process.env.MYSQL_DATABASE + '.`speakers` ( `id` INT NOT NULL AUTO_INCREMENT, `date` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP , `name` TEXT NOT NULL, `gender` TEXT NOT NULL, `active` TINYINT(1) DEFAULT 0, PRIMARY KEY (id))'
  )
  ...
  await executeSqlQueryWithoutValues(
    'CREATE TABLE IF NOT EXISTS ' + process.env.MYSQL_DATABASE + '.`settings` ( `setting` VARCHAR(10) NOT NULL , `bool` TINYINT(1) NOT NULL , UNIQUE (`setting`))'
  )
  ...
  await executeSqlQueryWithoutValues(
    'INSERT IGNORE INTO ' + process.env.MYSQL_DATABASE + '.`settings`(`setting`, `bool`) VALUES (\'quotation\', 0)'
  )
}
